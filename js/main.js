...
function Blob(game, x, y) {
    ...
    this.anchor.set(0.5);
    // animation
    this.animations.add('crawl', [0, 1, 2], 2, true);
    this.animations.add('die', [0, 6, 0, 6, 0, 6, 3, 3, 4, 4, 5, 5], 12);
    this.animations.play('crawl');

    // physic properties
    this.game.physics.enable(this);
    this.body.collideWorldBounds = true;
    this.body.velocity.x = Blob.SPEED;
}
...
Blob.prototype.update = function () {
    ...
    if (this.body.touching.right || this.body.blocked.right) {
        this.body.velocity.x = -Blob.SPEED; // turn left
        this.scale.x = -1;
    }
    else if (this.body.touching.left || this.body.blocked.left) {
        this.body.velocity.x = Blob.SPEED; // turn right
        this.scale.x = 1;
    }
};

Blob.prototype.die = function () {
    this.body.enable = false;
...
LoadingState.preload = function () {
    ...
    this.game.load.spritesheet('blob', 'images/blob.png', 42, 32);
    this.game.load.spritesheet('blob', 'images/blob.png', 42, 32);
...
PlayState._handleCollisions = function () {
    this.game.physics.arcade.collide(this.blobs, this.platforms);
    this.game.physics.arcade.collide(this.blobs, this.enemyWalls);
    ...
    this.game.physics.arcade.overlap(this.hero, this.blobs,
...
PlayState._loadLevel = function (data) {
    ...
    this._spawnCharacters({hero: data.hero, blobs: data.blobs});
...
PlayState._spawnCharacters = function (data) {
    ...
    data.blobs.forEach(function (blob) {
        let sprite = new Blob(this.game, blob.x, blob.y);
        this.blobs.add(sprite);
    }, this);
